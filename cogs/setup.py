...
class Setup(commands.Cog):
    ...
    async def set_up(self, ctx, gathering_channel: discord.VoiceChannel,
            team_1: discord.VoiceChannel,
            team_2: discord.VoiceChannel,
            neutral: discord.VoiceChannel):
        try:
            db.setup_server(ctx.message.guild.id, gathering_channel.id, team_1.id, team_2.id, neutral.id)
